...
module.exports = function(source) {
  ...
  // 注释Tag 开始结束的名称和长度
  const startTag = '<!--ac-demo:'
  const startTagLen = startTag.length
  const endTag = ':ac-demo-->'
  ...
  while (commentStart !== -1 && commentEnd !== -1) {
    ...
    const demoComponentName = `ac-demo${id}`  // 内联组件名称
    output.push(`<template #source><${demoComponentName} /></template>`)  // 使用slot插槽 运行组件
